...
public class DAO_sv {
    ...
    public boolean updateSinhVien(Account acc) {
        String sql = "UPDATE tbl_Account SET HOTEN = ?, GIOITINH = ?, NGAYSINH = ?, QUEQUAN = ?, NGANH = ?,"
                + " TRUONG = ?, EMAIL = ?, MATKHAU = ?, DSHOCKY = ? WHERE SDT = ?";
        try {
            PreparedStatement ps = conn.prepareStatement(sql);
    ...
    public ArrayList< Account> timSvTheoTruong(String ten) throws UnsupportedEncodingException {
        ArrayList< Account> accList = new DAO_sv().getAccSvList();
        ArrayList< Account> accList2 = new ArrayList<>();
        ...
        // Tìm kiếm trong danh sách các trường học
        for (Account acc : accList) {
            if (acc.getTruong().toLowerCase().contains(searchTermUTF8.toLowerCase())) { // So sánh chuỗi tìm kiếm với tên 
                accList2.add(acc);
            }
        }
        return accList2;
...
//                ps.setFloat(2, gra.getDiemgk().floatValue());
 ...
 /// test   
    public Account2 getAcc2Bysdt(String sdt) {
        Account2 student = null;
        try {
            // Tạo câu truy vấn
            String query = "SELECT * FROM tbl_Account2 WHERE SDT = ?";
            PreparedStatement statement = conn.prepareStatement(query);
            statement.setString(1, sdt);
            ResultSet resultSet = statement.executeQuery();
            // Lấy thông tin sinh viên từ kết quả truy vấn
            if (resultSet.next()) {
                student = new Account2();
                student.setSdt(resultSet.getString("SDT"));
                student.setName(resultSet.getString("HOTEN"));
...
/// test
        public boolean updateAcc2Img(Account2 acc, byte[] photo) {
            String sql = "UPDATE tbl_Account2 SET AVATAR = ? "
                    + "WHERE SDT = ?";
            try {
                PreparedStatement ps = conn.prepareStatement(sql);
                ps.setBytes(1, photo);
...
// real
    ...
    public double getDTB_Truong(String idmon, Account acc){
        double diem = 0;
        double sodiem = 0;
        ArrayList< Grade> gradeList = new ArrayList<>();
        String sql = "SELECT * FROM tbl_Grade WHERE IDMON= ?";
        try {
            ...
            for (Grade grade : gradeList) {
                ...
                if(grade.getDiemtbhe10() != null){
                    if(acc.getTruong().equals(account.getTruong())){
                        diem += grade.getDiemtbhe10();
                        sodiem += 1;                        
                    }
                }
